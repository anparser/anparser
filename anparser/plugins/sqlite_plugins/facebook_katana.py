...
def facebook_katana(file_list):
    ...
    if folder_counts_data:
        for entry in folder_counts_data:
            ...
            try:
                katana_data['Last Seen'] = time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.gmtime(entry['last_seen_time'] / 1000.))
            except TypeError:
    ...
    if folders_data:
        for entry in folders_data:
            ...
            katana_data['Name'] = ''
            try:
                katana_data['Timestamp'] = time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.gmtime(entry['timestamp_ms'] / 1000.))
            except TypeError:
                katana_data['Timestamp'] = ''
            katana_data['Last Seen'] = ''
            katana_data['Last Fetch Time'] = ''
    ...
    if threads_data:
        for entry in threads_data:
            ...
            katana_data['Last Seen'] = ''
            try:
                katana_data['Last Fetch Time'] = time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.gmtime(entry['last_fetch_time_ms'] / 1000.))
            except TypeError:
                katana_data['Last Fetch Time'] = ''
    ...
    if messages_data:
        for entry in messages_data:
            ...
            try:
                ...
                katana_data['Timestamp'] = ''
            try:
                katana_data['Timestamp Sent'] = time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.gmtime(entry['timestamp_sent_ms'] / 1000.))
            except TypeError:
